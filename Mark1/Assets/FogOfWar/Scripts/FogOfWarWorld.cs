...
namespace FOW
{
    ...
    public class FogOfWarWorld : MonoBehaviour
    {
        ...
        public float MaxFogRegrowAmount = .5f;
        ...
        public float ConstantTextureBlurAmount = 0.3f;
            ...
            switch (revealerMode)
            {
                ...
                case RevealerUpdateMode.N_Per_Frame:
                    for (int i = 0; i < Mathf.Clamp(numRevealersPerFrame, 0, numDynamicRevealers); i++)
                    {
                        ...
                        Revealers[currentIndex].RevealHiders();
        ...
        void SetMaterialBounds()
        {
            ...
            if (FogOfWarMaterial != null)
                FogOfWarMaterial.SetVector("_worldBounds", new Vector4(WorldBounds.size.x, WorldBounds.center.x, WorldBounds.size.z, WorldBounds.center.z));
            if (FowTextureMaterial != null)
                FowTextureMaterial.SetVector("_worldBounds", new Vector4(WorldBounds.size.x, WorldBounds.center.x, WorldBounds.size.z, WorldBounds.center.z));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (newUseMiniMap || fow.FOWSamplingMode == FogOfWarWorld.FogSampleMode.Texture || fow.FOWSamplingMode == FogOfWarWorld.FogSampleMode.Both)
            {
                ...
                if (newUseRegrow)
                {
                    ...
                    float newRegrowSpeed = EditorGUILayout.Slider("Fog Regrow Speed: ", oldRegrowSpeed, 0, 2);
